  ...
  return (
    <div className="bg-white p-6 min-h-screen">
      <div className="flex justify-between items-center mb-6 print:hidden">
        <h1 className="text-2xl font-bold">Lower Primary Timetable</h1>
        <button
          onClick={handlePrint}
      ...
      <div id="print-area">
        <table className="w-full  border-collapse border border-blue-950 text-center">
          <thead>
            <tr>
              <th className="border p-2 border-blue-950 bg-blue-100">Day / Time</th>
              {timeSlots.map((time) => (
          ...
          <tbody>
            {days.map((day) => (
              <tr key={day}>
                <td className="border border-blue-950 p-2 font-semibold bg-yellow-100">{day}</td>
                {timeSlots.map((time) => {
